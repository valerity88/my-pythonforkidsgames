...
class Game:
    ...
    def __init__(self):
        self.tk = Tk()
        self.tk.title("Человек спешит к выходу")
...
class MovingPlatformSprite(Sprite):
    def __init__(self, game):
        Sprite.__init__(self, game)
        self.photo_image = PhotoImage(file = "platform1.gif")
        self.image = game.canvas.create_image(300, 400, \
                image = self.photo_image, anchor = 'nw')
    ...
    def move(self):
        ...
        if co.x2 >= self.game.canvas_width:
            self.x = -1
...
class StickFigureSprite(Sprite):
    ...
    def move(self):
        ...
        for sprite in self.game.sprites:
            ...
            if sprite.endgame and co.x2 == sprite_co.x1:
...
class DoorSprite(Sprite):
    ...
    def __init__(self, game):
        Sprite.__init__(self, game)
        self.images_door = [
            PhotoImage(file = "door1.gif"),
            PhotoImage(file = "door2.gif")
            ]       
        self.image = game.canvas.create_image(45, 30, \
                image = self.images_door[0], anchor = 'nw')
        self.coordinates = Coords() 
        
    def coords(self):
        xy = self.game.canvas.coords(self.image)
        self.coordinates.x1 = xy[0]+14
        self.coordinates.y1 = xy[1]
        self.coordinates.x2 = xy[0] + 14
        self.coordinates.y2 = xy[1] + 30
        return self.coordinates
    ...
    def move(self):         
        ...
        for sprite in self.game.sprites:
            ...
            if collided_right(co, sprite_co) \
                   or collided_top(co, sprite_co):
                self.game.canvas.itemconfig(self.image, \
                        image = self.images_door[1])
                self.game.canvas.create_text(250, 250, \
                        text = 'ВЫ ПОБЕДИЛИ!!!', font = ('Helvetica', 30), \
                                                 fill = 'black')
                self.endgame = True
...
platform2 =  PlatformSprite(g, PhotoImage(file = "platform1.gif"), \
                            150, 440, 100, 10)
platform3 =  MovingPlatformSprite(g)
